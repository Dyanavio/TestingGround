...
{
    public class MonoCurrencyExchange : ICurrencyExchange
    {
        public async Task<List<CurrencyExchange>> GetCurrencyExchangesAsync()
        {
            ...
            var monoExchange = JsonSerializer.Deserialize<List<MonoExchange>>(json)!;
            return monoExchange.Select(mono => new CurrencyExchange
            {
                FullName = iso4217[mono.currencyCodeA].FullName,
                ShortName = iso4217[mono.currencyCodeA].ShortName,
                RateBuy = mono.rateBuy ?? mono.rateCross ?? 0.0,
                RateSell = mono.rateSell ?? mono.rateCross ?? 0.0,
                Date = DateOnly.FromDateTime(DateTime.FromBinary(mono.date))
            }).ToList();
        }
        // ORM
        /*
        ...
        private class CurrencyInfo
        {
            ...
            public CurrencyInfo(string fullName, string shortName)
            {
                FullName = fullName;
                ShortName = shortName;
            }
        ...
        private Dictionary<int, CurrencyInfo> iso4217 = new Dictionary<int, CurrencyInfo>
        {
